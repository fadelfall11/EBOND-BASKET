        ...
        <div style="max-width: 800px; margin: 0 auto;">
            ...
            <div class="card">
                <div class="card-content">
                    <h2 class="card-title" style="margin-bottom: 2rem;">Ajouter un entraînement</h2>
                    ...
                    <form action="{{ route('emploi-du-temps.store') }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select name="categorie_id" class="form-input" required>
                                ...
                                @foreach($categories as $categorie)
                                    <option value="{{ $categorie->id }}" {{ old('categorie_id') == $categorie->id || request('categorie') == $categorie->id ? 'selected' : '' }}>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                            <div class="form-group">
                                ...
                                <input type="time" name="heure_debut" class="form-input" value="{{ old('heure_debut') }}" required>
                                @error('heure_debut')
                                    <div class="form-error">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="time" name="heure_fin" class="form-input" value="{{ old('heure_fin') }}" required>
                                @error('heure_fin')
                                    <div class="form-error">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="lieu" class="form-input" value="{{ old('lieu') }}" placeholder="Ex: Terrain principal">
                            @error('lieu')
                                <div class="form-error">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="description" class="form-input" rows="4" placeholder="Description de l'entraînement...">{{ old('description') }}</textarea>
                            @error('description')
                                <div class="form-error">{{ $message }}</div>
                            @enderror
                        </div>
