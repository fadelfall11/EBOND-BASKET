...
<section class="section">
    <div class="container">
        <div style="display:flex; align-items:center; justify-content:space-between; gap:1rem; flex-wrap:wrap; margin-bottom:2rem;">
            <div>
                <h2 class="section-title" style="margin-bottom:0.25rem;">Gestion des actualités</h2>
        ...
        <div class="card">
            <div class="card-content">
                    ...
                    <table style="width:100%; border-collapse: collapse;">
                        <thead>
                            <tr style="text-align:left; border-bottom:1px solid #e2e8f0;">
                                ...
                                <th style="padding:0.75rem;">Auteur</th>
                        ...
                        <tbody>
                            @forelse($actualites as $actualite)
                                <tr style="border-bottom:1px solid #f1f5f9;">
                                    <td style="padding:0.75rem; font-weight:600;">{{ $actualite->titre }}</td>
                                    <td style="padding:0.75rem; color:#64748b;">{{ $actualite->date_publication?->format('d/m/Y H:i') }}</td>
                                    <td style="padding:0.75rem; color:#64748b;">{{ $actualite->auteur ?? '-' }}</td>
                                    <td style="padding:0.75rem;">
                                        <div style="display:flex; gap:0.5rem; flex-wrap:wrap;">
                                            <a class="btn-outline" href="{{ route('admin.actualites.edit', $actualite) }}" style="border:2px solid #2563eb; color:#2563eb; padding:0.5rem 0.9rem; border-radius:10px; text-decoration:none; display:inline-flex; align-items:center; gap:0.4rem;">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" style="padding:1rem; color:#64748b; text-align:center;">Aucune actualité.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                @if($actualites->hasPages())
                    <div style="margin-top:1.5rem;">{{ $actualites->links() }}</div>
                @endif
            </div>
        </div>
    </div>
