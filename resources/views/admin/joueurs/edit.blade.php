    ...
    <div class="container">
        <div style="max-width: 900px; margin: 0 auto;">
            <div style="display:flex; align-items:center; justify-content:space-between; gap:1rem; flex-wrap:wrap; margin-bottom:2rem;">
                <div>
                    <h2 class="section-title" style="margin-bottom:0.25rem;">Modifier un joueur</h2>
            ...
            <div class="card">
                <div class="card-content">
                    <form method="POST" action="{{ route('admin.joueurs.update', $joueur) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:1rem;">
                            <div class="form-group">
                                ...
                                <input class="form-input" name="prenom" value="{{ old('prenom', $joueur->prenom) }}" required>
                                @error('prenom')<div class="form-error">{{ $message }}</div>@enderror
                            </div>
                            <div class="form-group">
                                <label class="form-label">Nom *</label>
                                <input class="form-input" name="nom" value="{{ old('nom', $joueur->nom) }}" required>
                        ...
                        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:1rem;">
                            <div class="form-group">
                                ...
                                <input class="form-input" name="poste" value="{{ old('poste', $joueur->poste) }}" required>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-input" name="categorie_id" required>
                                    @foreach($categories as $cat)
                                        <option value="{{ $cat->id }}" {{ (string) old('categorie_id', $joueur->categorie_id) === (string) $cat->id ? 'selected' : '' }}>
                                            {{ $cat->nom }} ({{ $cat->genre }})
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:1rem;">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-input" name="numero" value="{{ old('numero', $joueur->numero) }}" required>
                            ...
                            <div class="form-group" style="display:flex; align-items:center; gap:0.75rem; padding-top:2rem;">
                                <input id="capitaine" type="checkbox" name="capitaine" value="1" {{ old('capitaine', $joueur->capitaine) ? 'checked' : '' }}>
                        ...
                        @include('components.photo-position-select', ['photoPosition' => $joueur->photo_position ?? 'center'])

                        <div class="form-group">
                            <label class="form-label">Éloges</label>
                            <textarea class="form-input" name="eloges" rows="6">{{ old('eloges', $joueur->eloges) }}</textarea>
